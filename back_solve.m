function solve = back_solve(inputTaskTableData, conditionsTableData, segBegin, segEnd, stepsCount, accuracyExternal, accuracyInternal, solvingMethodForInternalTask, stepSize, solvingMethodForExternalTask, timeOfT, initialVectorTableData)
        ftx = inputTaskTableData;
        ftx = ftx(1:end-1, 2);
        n = size(ftx, 1);
        
        dftx = back_findDftx(ftx);
        XMatrix = back_generateXMatrix(size(ftx, 1));
        initConditionForXMatrix = back_generateInitConditionForXMatrix(size(ftx, 1))';
        ...
        toFile = [ftx; strDX];
        back_writeSystemToFile(toFile);
        charFP = conditionsTableData;
        FP = back_makeFP(charFP, segBegin, segEnd);
        ...
        for i = 1:stepsCount
            ...
            for j = 0:stepSize:(1-stepSize)
                ...
                switch solvingMethodForExternalTask
                    ...
                    case 3
                        [mu, p] = ode23t(@back_findDFP, [j (j+stepSize)], p0, odeset('RelTol', accuracyExternal));
                    case 4
                        [mu, p] = ode113(@back_findDFP, [j (j+stepSize)], p0, odeset('RelTol', accuracyExternal));
                    case 5
                        [mu, p] = ode23s(@back_findDFP, [j (j+stepSize)], p0, odeset('RelTol', accuracyExternal));
                    case 6
